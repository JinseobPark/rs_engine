...
void main() {
    ...
    float alpha = texture(albedo_map, TexCoords).a;
...
//    vec3 lighting = (kD * Albedo / PI + specular) * diff * NdotL;
    ...
    bool ssr_passed = false;
    if(b_ssr_reflection)
    {
        // ssr reflection
        float ssr_alpha = texture(ssr_map, TexCoords).a;
        specularIBL += textureLod(ssr_map, TexCoords, roughness * 5.0).rgb * F * ssr_alpha;
        if(texture(ssr_map, TexCoords).a > 0.01)
            ssr_passed = true;
    }

    //b_skybox_reflection
    // skybox reflection
    if(b_skybox_reflection && !ssr_passed)
    {
    ...
    vec3 final_color = ambient + (1.0 - shadow) * lighting + specularIBL;
